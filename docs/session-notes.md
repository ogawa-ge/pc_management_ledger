...
- **対応**: `pcs/page.tsx` を修正し、PC新規登録への遷移ボタンおよび未使用PC一覧への遷移ボタンを追加。また、CSVダウンロード機能 of ボタンについては、管理者 (`(session.user as any).role === 'Admin'`) のみに限定して表示されるようにアクセス制限を実装。
- **効果**: 管理者はもちろん、一般ユーザーもPC一覧画面から「自身のPC登録」「未使用PC一覧の確認」へスムーズに遷移できるようになりました。

##### 8. PC新規登録時のオーナーユーザー選択バグの修正 ✅ COMPLETED
- **課題**: PC新規登録画面を開いても、オーナーユーザーの選択肢（`<select>`）が画面に表示されず、登録を進めようとすると「オーナーを選択してください」というバリデーションエラーが発生する問題。
- **原因**: フロントエンドの PC 登録フォーム内 (`frontend/src/app/pcs/register/page.tsx`) に、オーナーユーザーの選択肢をレンダリングする UI フィールドそのものが欠落していた。
- **対応**: `register/page.tsx` に `useSession` を導入し、ログインユーザーが管理者の場合は全ユーザーから代理登録先をドロップダウン選択でき、一般ユーザーの場合は自身のユーザー情報を自動セットして表示する UI を実装。これにより、オーナーユーザーの正常な設定・送信が完了するようになりました。

##### 9. 次回作業 📌 NEXT STEP
- ユーザーからのフィードバックにより、**「未使用PC一覧画面 (`/pcs/unused`) への遷移時エラー（502または500）」が依然として解消されていない**ことが確認された。
- 以前、バックエンド Lambda 側 (`backend/lambda/src/main.py`) のクエリパラメータ転送部分における `urllib3.request.urlencode` 指定バグを `urllib.parse.urlencode` に修正して再デプロイしたが、依然としてエラーが続いているため、以下の調査・解決を次回のセッション開始時に最優先で実施する。
  1. **CloudWatch の最新エラーログの再抽出**: `fetch_logs.py` を用いて、未使用PC一覧にアクセスした際の 502/500 のスタックトレースを正確にダンプし、新たな ModuleNotFoundError や AttributeError、転送先の ECS からのコールドスタート応答不備がないか確認する。
  2. **ECS 側のクエリパラメータ解釈ロジックの確認**: バックエンド ECS コンテナ（FastAPI）で、未使用PC一覧のクエリパラメータ `?status=Unused` がどのように処理されているか、またスキーマやDynamoDBスキャン時に不整合が起きていないか検証し、完全に解消する。
